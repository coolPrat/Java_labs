 ...
 * StorageFixed.java
 *
 * Version: 1.0: StorageFixed.java,v 1.0 9/26/2015 10:19:23
 *
...
public class StorageFixed<E, V> implements Storage<E, V> {
    /**
     * The class StorageFixed can store upto 100 elements of type Element which
    ...
    private int maxElements = 100; // maximum number of elements that can be stored.
    private StorageIterator iterator = new StorageIterator(this);
    // Iterator used to traverse the list

    /**
     * Sets the maximum number of elements that can be stored.
     * @param maxElements    maximum number of elements that can be stored.
     */
    ...
    /**
     * returns the maximum number of elements that can be stored in the storage
     * @return    (int)maximum number of elements that can be stored
     */
    ...
    public boolean add(E element) {
        ...
        if (elementCount == 0) {
            ...
            ++elementCount;
            return true;
        } else if(elementCount < maxElements) {
    ...
    public boolean add(int index, E element) {
        ...
        if(index > 0 && index < maxElements && elementCount < maxElements) {
            if (index > 0) {
                int counter = -1;
    ...
    @SuppressWarnings("unchecked")
    public Object clone() {
        StorageFixed<E, V> storage = new StorageFixed<>();
        StorageIterator iterator = new StorageIterator(storage);
        storage.setIterator(iterator);
        storage.setMaxElements(this.getMaxElements());
        Element<E, V> element;
        int count = 0;
